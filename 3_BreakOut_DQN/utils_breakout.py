...
def preprocess_batch(batch, bs, n, h, w, device):
    ...
    for i in range(bs):
        # batch[i] is a tuple; [0] then is the state, [1] the action and so on..
        states[i, :, :, :] = batch[i][0]
...
class CNN(nn.Module):
    def __init__(self, num_images):
        ...
        self.conv_3 = nn.Conv2d(in_channels=64, out_channels=64, kernel_size=3, stride=1, padding='same')
        ...
        self.relu_4 = nn.ReLU()
        self.maxp_4 = nn.MaxPool2d(kernel_size=2, stride=2)
...
class Net(nn.Module):
    def __init__(self, num_images, img_width, img_height, num_actions):
        ...
        self.fc_1 = nn.Linear(self.cnn_out_size, 512)
...
class Logger():
    ...
    def log_metrics(self, current_episode):
        ...
        self.writer.add_scalar('Epsilon', self.episode_epsilons[-1], current_episode)
